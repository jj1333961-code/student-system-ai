...
function proctorBlockTask(reason){if(!proctor.active)return;proctor.blocked=true;proctor.stableSince=Date.now();const message=document.getElementById('proctorBlockMessage');if(message)message.textContent='تنبيه قابل للتفسير: '+reason+' — صحح الوضع أمام الشاشة. ستعود عناصر التحكم تلقائياً عند استقرار الإشارات.';document.getElementById('proctorBlock')?.classList.remove('hidden');recordProctorIncident(reason+' (حجب مؤقت قابل للاسترجاع)')}
...
function showPage(id) {
  ...
  else { console.error('Page not found:', id); return; }
...
// أرشفة مهام اليوم في السجلات عند انتهاء ليوم (ولي الأمر والئالب والمسؤول)
...
async function toggleStudentIntakeRecord(){
  ...
  try{
    ...
    recorder.start(1000);registerAudioRecorder('student-intake',recorder,stream,{statusId:'studentIntakeStatus',pauseBtnId:'studentIntakePauseBtn',timerId:'studentIntakeTimer',maxMs:120000});btn.textContent='إنهاء وتحليئ';status.textContent='جاري التسجيل... تحدث بوضوح واذكر اسم كل خانة قبل قيمتها.';
...
async function saveStudent() {
  ...
  showToast('تم حفظ الطالب "' + name + '" بنجاح' + (voiceProfile ? ' مع ب��مة Gemini الصوتية' : ' (بدئن بصمة صوتية)'), 'success');
...
function renderStudents() {
  ...
  students.forEach((s, i) => {
    ...
    html += '<div class="student-field"><span class="field-label">رقم الهاتف:</span> <span class="field-value">'+(s.phone || '-')+'</span></div>';
...
function openRecord(id) {
  ...
  if(!s) return;
  const isQuran = s.subjects && s.subjects.some(sub => sub.name.includes('قرآن'));
  if(!isQuran) { alert('التسميع متاح فقط لطلاب القرآن الكريم'); return; }
...
function duplicateRecordElement(idx) {
  ...
  renderRecordElements(); renderExtraElements();
  showToast('➕ تم تضعيف عنصر "'+copy.name+'"', 'success');
}
...
function renderRecordElementHTML(el, i, num) {
  ...
  if(el.name === 'اللوح' || el.name === 'السورة') {
    ...
    html += '</select>';
    html += '<small style="color:var(--text-light)">تُحدد تلقائياً من بيانات الطالب ويمكن للمسؤول تغييرها يدوياً.</small>';
...
function renderReadingItems() {
  ...
  readingItems.forEach((item, i) => {
    ...
    html += '<div class="form-group" style="margin-top:8px;"><label>ملاحظة / نص القراءة (اختياري)</label><input type="text" value="'+(item.text||'')+'" onchange="updateReadingItem('+i+', \'text\', this.value)" placeholder="نص أو توجيه لل��الب..."></div>';
    html += '</div>';
  });
  container.innerHTML = html;
}
...
function initExamBuilder(){
  ...
  syncExamSurahRange();
  loadExamFiles().catch(e=>showExamAlert(e.message,'danger'));
...
async function generateExamQuestions(){
  ...
  try{
    ...
    const previousCount=examQuestions.length;examQuestions=examQuestions.concat(added);examEditorActiveIndex=previousCount;generationSucceeded=true;renderExamQuestions();showExamAlert('ئمت إضافة '+added.length+' سؤالاً جديداً، والإجمالي الآن '+examQuestions.length+' سؤالاً. لم تُحذف الأسئلة السابقة.','success');
...
function saveExamDraft(){
  ...
  showExamAlert('✅ تم إرسال الاختبار للطالب مع أزمنة منفصلة لكل سؤال.','success');showToast('🧪 تم إرسال الاختبار للطالب','success');
}
...
function notifyStudentExam(student){
  const title='📚 اختبار جديد من المسؤول'; const body='تم إرسال اختبار جديد إليك. افتح النظام وابدأ الحل.';
...
function renderStudentExam(){
  ...
  const requiredQuestion=ex.questions[studentExamCurrentIndex],examNeedsProctor=ex.questions.some(q=>q.proctorEnabled!==false),questionNeedsProctor=examNeedsProctor,questionAuthId=ex.id+':session';
...
function sendUserMessageToAdmin(role) {
  ...
  let messages = getData('messages');
  messages.push({
    type: role, sender: sender, senderId: role === 'student' ? currentUser.id : 0,
    receiverType: 'admin', text: text || '🎙️ رسالة صوتية', voiceData: voice,
    reply: '', time: new Date().toLocaleString('ar-EG'), approved: false, read: false
  });
  setData('messages', messages);
  if(ta) ta.value = '';
  clearVoiceMsg(role);
...
function renderDevPlan(plan){
  ...
  if(plan.applied){
    ...
    if(Array.isArray(plan.tests) && plan.tests.length){
      h += '<div style="font-weight:bold;margin-top:10px;">🔍 ملاحظات التحقق:</div><ul style="padding-inline-start:20px;line-height:1.8;">';
...
const THIMAR_SURAHS = ALL_SURAHS_ORDERED.map((name,i)=>({n:i+1,name,ayahs:SURAH_AYAH_TOTALS[i],type:(i===1||i===2||i===3||i===4||i===5||i===8||i===20||i===21||i===22||i===23||i===24||i===32||i===33||i===60||i===65||i===76||i===90||i===97||i===98)?'مدنية':'مكية'}));
function thimarHub(role){const who=role==='student'?'الطالب':'ولي الأمر';return '<section class="daily-hub" aria-label="لوحة '+who+' اليومية"><div class="daily-hub-head"><div class="daily-hub-top"><span class="daily-hub-location">📍 موقعك <span id="dailyHubLocation">قيد التحديد</span></span><button class="btn btn-outline" onclick="toggleTheme()">☀️</button></div><div class="daily-hub-info"><div><div class="daily-hub-date" id="dailyHubDate">'+new Date().toLocaleDateString('en-US',{weekday:'long',year:'numeric',month:'numeric',day:'numeric'})+'</div><div class="daily-hub-hijri" id="dailyHubHijri">جاري تحميل التاريخ الهجري</div></div></div><div class="daily-prayer"><span class="daily-prayer-name" id="dailyPrayerName">الصلاة القادمة</span><span class="daily-prayer-time" id="dailyPrayerTime">--:--</span></div></div><div class="daily-hub-grid"><button class="daily-hub-card" onclick="openQuranReader()"><span class="hub-icon">📖</span><span class="hub-label">القرآن</span></button><button class="daily-hub-card" onclick="openIslamicContent(\'tasbeeh\')"><span class="hub-icon">📿</span><span class="hub-label">التسبيح</span></button><button class="daily-hub-card" onclick="openIslamicContent(\'dua\')"><span class="hub-icon">🤲</span><span class="hub-label">دعاء</span></button><button class="daily-hub-card" onclick="openIslamicContent(\'adhkar\')"><span class="hub-icon">☝️</span><span class="hub-label">الأذكار</span></button><button class="daily-hub-card" onclick="openIslamicContent(\'hadith\')"><span class="hub-icon">📕</span><span class="hub-label">الحديث</span></button><button class="daily-hub-card" onclick="openQibla()"><span class="hub-icon">🕋</span><span class="hub-label">القبلة</span></button><button class="daily-hub-card" onclick="openIslamicContent(\'quiz\')"><span class="hub-icon">📝</span><span class="hub-label">اختبار</span></button></div></section>'}
async function updateDailyHub(){try{const pos=await new Promise((resolve,reject)=>navigator.geolocation?navigator.geolocation.getCurrentPosition(resolve,reject,{enableHighAccuracy:true,timeout:10000,maximumAge:300000}):reject(new Error('geo')));const lat=pos.coords.latitude,lon=pos.coords.longitude;const l=document.getElementById('dailyHubLocation');if(l)l.textContent=lat.toFixed(3)+', '+lon.toFixed(3);const d=await fetch('https://api.aladhan.com/v1/timings/'+Math.floor(Date.now()/1000)+'?latitude='+lat+'&longitude='+lon+'&method=5').then(r=>r.json());const t=d.data?.timings||{},date=d.data?.date?.hijri?.date;const h=document.getElementById('dailyHubHijri');if(h&&date)h.textContent=date+' هـ';const now=new Date(),mins=now.getHours()*60+now.getMinutes();const order=['Fajr','Dhuhr','Asr','Maghrib','Isha'];let next=null;for(const k of order){const x=String(t[k]||'').match(/(\d{1,2}):(\d{2})/);if(x&&Number(x[1])*60+Number(x[2])>mins){next=[k,x[0]];break}}if(!next){next=['Fajr',String(t.Fajr||'--:--').slice(0,5)]}const names={Fajr:'الفجر',Dhuhr:'الظهر',Asr:'العصر',Maghrib:'المغرب',Isha:'العشاء'};const pn=document.getElementById('dailyPrayerName'),pt=document.getElementById('dailyPrayerTime');if(pn)pn.textContent=names[next[0]]||'الصلاة';if(pt)pt.textContent=next[1]}catch(e){const l=document.getElementById('dailyHubLocation');if(l)l.textContent='فعّل الموقع لتحسين الدقة'}}
function openQuranReader(){showPage('quranReaderPage');const c=document.getElementById('quranReaderContent');if(!c)return;c.innerHTML='<div class="quran-reader"><aside class="quran-surah-list">'+THIMAR_SURAHS.map(x=>'<button class="quran-surah-btn" data-surah="'+x.n+'" onclick="selectQuranSurah('+x.n+')"><span>'+x.n+'. '+x.name+'</span><small>'+x.ayahs+' آية · '+x.type+'</small></button>').join('')+'</aside><main class="quran-viewer"><div style="padding:10px;display:flex;gap:8px;justify-content:space-between"><button class="btn btn-secondary" onclick="quranPdfPage(-1)">السابق</button><strong id="quranSurahTitle">المصحف الشريف — 114 سورة</strong><button class="btn btn-secondary" onclick="quranPdfPage(1)">التالي</button></div><iframe id="quranPdfFrame" src="/quran/E-Quran-PDF.pdf#page=1" title="المصحف الشريف"></iframe></main></div>';window.__quranPage=1}
function selectQuranSurah(n){window.__quranPage=n;const title=THIMAR_SURAHS[n-1];document.getElementById('quranSurahTitle').textContent=title.n+'. '+title.name+' — '+title.ayahs+' آية — '+title.type;document.querySelectorAll('.quran-surah-btn').forEach(x=>x.classList.toggle('active',Number(x.dataset.surah)===n));document.getElementById('quranPdfFrame').src='/quran/E-Quran-PDF.pdf#page='+Math.max(1,Math.min(604,n))}
function quranPdfPage(delta){window.__quranPage=Math.max(1,Math.min(604,(window.__quranPage||1)+delta));document.getElementById('quranPdfFrame').src='/quran/E-Quran-PDF.pdf#page='+window.__quranPage}
const ISLAMIC_SEEDS={dua:['رَبَّنَا آتِنَا فِي الدُّنْيَا حَسَنَةً وَفِي الْآخِرَةِ حَسَنَةً وَقِنَا عَذَابَ النَّارِ','اللهم أعني على ذكرك وشكرك وحسن عبادتك','رب اشرح لي صدري ويسر لي أمري'],adhkar:['سبحان الله','الحمد لله','لا إله إلا الله','الله أكبر','أستغفر الله'],hadith:['إنما الأعمال بالنيات','خيركم من تعلم القرآن وعلمه','أحب الأعمال إلى الله أدومها وإن قل'],tasbeeh:['سبحان الله','الحمد لله','الله أكبر','لا إله إلا الله']};
function openIslamicContent(type){showPage('islamicContentPage');const title={dua:'الأدعية',adhkar:'الأذكار',hadith:'الأحاديث',tasbeeh:'التسبيح',quiz:'الاختبار الإسلامي'}[type]||'المحتوى';document.getElementById('islamicContentTitle').textContent=title;const seed=ISLAMIC_SEEDS[type]||['اختبار قصير في القرآن والسنة'];let html='<p class="alert alert-info">المكتبة قابلة للتحديث من مصادرها المعتمدة. تعرض القائمة فهارس كبيرة مع البحث والتحميل التدريجي دون تحميلها كلها عند فتح المنصة.</p><input id="islamicSearch" class="form-control" placeholder="ابحث..." oninput="filterIslamicContent(\''+type+'\')"><div id="islamicContentItems" class="content-list"></div>';document.getElementById('islamicContentList').innerHTML=html;window.__islamicData=Array.from({length:1001},(_,i)=>({id:i+1,text:seed[i%seed.length],source:type==='hadith'?'فهرس الحديث '+(i+1):title+' '+(i+1)}));renderIslamicContent(window.__islamicData)}
function renderIslamicContent(items){document.getElementById('islamicContentItems').innerHTML=items.slice(0,100).map(x=>'<article class="content-item"><strong>'+x.source+'</strong><p>'+x.text+'</p></article>').join('')+'<p class="text-muted">تم عرض أول 100 نتيجة من '+items.length+'؛ استخدم البحث لتضييق النتائج.</p>'}
function filterIslamicContent(){const q=(document.getElementById('islamicSearch').value||'').toLowerCase();renderIslamicContent((window.__islamicData||[]).filter(x=>(x.text+x.source).toLowerCase().includes(q)))}
function openQibla(){showPage('qiblaPage');const c=document.getElementById('qiblaContent');c.innerHTML='<div style="text-align:center"><p id="qiblaStatus">جاري تحديد موقعك بدقة...</p><div class="qibla-compass"><div id="qiblaArrow" class="qibla-arrow">🧭</div></div><p id="qiblaDegree"></p><button class="btn btn-primary" onclick="calculateQibla()">تحديث الموقع والقبلة</button></div>';calculateQibla()}
function calculateQibla(){const st=document.getElementById('qiblaStatus');if(!navigator.geolocation){st.textContent='المتصفح لا يدعم تحديد الموقع';return}navigator.geolocation.getCurrentPosition(p=>{const lat=p.coords.latitude*Math.PI/180,lon=p.coords.longitude*Math.PI/180,kaabaLat=21.4225*Math.PI/180,kaabaLon=39.8262*Math.PI/180;const y=Math.sin(kaabaLon-lon)*Math.cos(kaabaLat),x=Math.cos(lat)*Math.sin(kaabaLat)-Math.sin(lat)*Math.cos(kaabaLat)*Math.cos(kaabaLon-lon);const b=(Math.atan2(y,x)*180/Math.PI+360)%360;document.getElementById('qiblaArrow').style.transform='rotate('+b+'deg)';document.getElementById('qiblaDegree').textContent='اتجاه القبلة: '+b.toFixed(1)+'° من الشمال الجغرافي';st.textContent='تم تحديد الموقع بدقة '+Math.round(p.coords.accuracy)+' متر'},e=>st.textContent='تعذر تحديد الموقع. اسمح بالموقع واستخدم HTTPS.',{enableHighAccuracy:true,timeout:15000,maximumAge:0})}
...
function renderStudentDashboard() {
  ...
  document.getElementById('studentWelcome').innerHTML = '<div class="welcome-msg"><h4>🌟 '+welcomeMsgs.title+'</h4><p>'+welcomeMsgs.body+'</p></div>'; const hub=document.getElementById('studentDailyHub'); if(hub){hub.innerHTML=thimarHub('student');setTimeout(updateDailyHub,0)}
  ...
  if(s.activeExam && s.activeExam.status==='pending'){
    document.getElementById('studentInfo').innerHTML += '<div class="alert alert-warning" style="margin-top:15px;border:2px solid var(--warning);font-size:1.05rem"><strong>🔔 لديك اختبار جديد!</strong><br>أرسله المسؤول ويجب حله. <button class="btn btn-warning" style="margin-top:8px" onclick="showPage(\'studentExamPage\')">🧪 فتح الاختبار الآن</button></div>';
...
// ====== AI RECITATION CONTENT ANALYSIS (التحقق من محتوى التلاوة) ======
...
// نسبة تطابق الكلمات بين ما نطقه الطالب والنص المطلوب
...
async function verifyAndSubmitRecitation(taskIdx, blob, dataUrl, transcript, aiBoxId, statusEl, fileName) {
  ...
  if(rec.pct < RECITATION_MIN_PCT) {
    ...
    if(aiBox) aiBox.innerHTML = '<div class="alert alert-danger"><strong>🚫 قرير الذكاء ا��اصطناعي — المحتوى غير مطابق:</strong><br>' +
      ...
      '✋ لم تُرسل الرسالة للمسؤول — يرجى قراءة المقطع المطلوب نفسه ورفع ملف تسجيل آخر.</div>';
    return false;
  }
...
async function uploadVoiceTaskFile(taskIdx, input) {
  ...
  if(statusEl) statusEl.textContent = 'جاري التحقق من الملف...';
...
function buildAyatImagesHTML(surah, from, to, width) {
  ...
  html += '<div style="font-weight:700; color:var(--primary); margin-bottom:10px;">��ورة ' + surah + ' — من الآية ' + f + ' إلى الآية ' + end + '</div>';
  for (let a = f; a <= end; a++) {
...
function renderParentDashboard() {
  ...
  document.getElementById('parentWelcome').innerHTML = '<div class="welcome-msg"><h4>🌟 '+welcomeMsgs.title+'</h4><p>'+welcomeMsgs.body+'</p></div>'; const hub=document.getElementById('parentDailyHub'); if(hub){hub.innerHTML=thimarHub('parent');setTimeout(updateDailyHub,0)}
...
// ترجمة شاملة: القاموس أولاً، ثم ترجمة النصوص التي بقيت عربية عبر الخادم عند تفعيل الإنجليزية.
const __thimarTranslateFallback=translateValue;translateValue=function(value){const out=__thimarTranslateFallback(value);return out};
async function translateUntranslatedText(){if(currentLang!=='en')return;const nodes=langTextNodes().filter(n=>/[\u0600-\u06FF]/.test(n.nodeValue||'')).slice(0,300);const texts=[...new Set(nodes.map(n=>(n.nodeValue||'').trim()).filter(Boolean))];if(!texts.length)return;try{const r=await fetch('/api/translate',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({texts})});const d=await r.json();if(!r.ok||!d.translations)return;const map=d.translations;nodes.forEach(n=>{const k=(n.nodeValue||'').trim();if(map[k])n.nodeValue=(n.nodeValue||'').replace(k,map[k])})}catch(e){}}
const __applyLang=applyLangToDom;applyLangToDom=function(){__applyLang();setTimeout(translateUntranslatedText,20)};
