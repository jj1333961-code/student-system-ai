...
export const defaultAntiCheatConfig: AntiCheatConfig = {
  ...
  blockMs: 8000, autoRestore: true, analysisIntervalMs: 150, sensitivity: 'medium', eyeTracking: true, maxWarnings: 3, recoveryStableMs: 2200, startStableMs: 1800, aiAssist: true,
}
...
const signalWeights: Record<string, number> = {
  ...
  'eyes-closed': 24, 'eye-occluded': 18, 'page-hidden': 14, 'window-blur': 10, 'fullscreen-exit': 12,
  'touch-missing': 20, 'multiple-touch': 28,
}
...
export function normalizeServerConfig(value: unknown): AntiCheatConfig {
  ...
  return {
    ...
    maxWarnings: Math.max(1, Math.min(20, Number(input.maxWarnings) || defaultAntiCheatConfig.maxWarnings!)),
    recoveryStableMs: Math.max(500, Math.min(60_000, Number(input.recoveryStableMs) || defaultAntiCheatConfig.recoveryStableMs!)),
    startStableMs: Math.max(500, Math.min(30_000, Number(input.startStableMs) || defaultAntiCheatConfig.startStableMs!)),
...
export function calculateRiskScore(signals: Signal[], previous = 0) {
  ...
  const persistence = Math.min(100, previous * (active.length ? 0.9 : 0.68))
