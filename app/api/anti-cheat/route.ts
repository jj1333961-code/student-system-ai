...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'ai-assist') {
      const signals = Array.isArray(body.signals) ? body.signals.slice(0, 12).map((x: any) => ({ type: String(x?.type || '').slice(0, 64), durationMs: clamp(x?.durationMs, 0, 60000) })) : []
      const riskScore = clamp(body.riskScore)
      const prompt = `أنت مساعد قرار لمراقبة اختبار. لا تحكم من إشارة واحدة ولا تطلب صورة أو فيديو. حلل فقط الإشارات الرقمية التالية وارجع JSON فقط: {decision: NORMAL|WARNING|SUSPICIOUS|CHEATING_DETECTED, confidence:0..1, reason:string}. score=${riskScore}, signals=${JSON.stringify(signals)}`
      let text = ''
      try {
        const key = process.env.GEMINI_API_KEY?.trim()
        if (key) { const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${key}`, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({contents:[{parts:[{text:prompt}]}],generationConfig:{temperature:0.1,responseMimeType:'application/json'}}), signal:AbortSignal.timeout(8000) }); const d:any=await r.json(); text=String(d?.candidates?.[0]?.content?.parts?.[0]?.text||'') }
      } catch {}
      if (!text) { try { const key=process.env.GROQ_API_KEY?.trim(); if(key){const r=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${key}`},body:JSON.stringify({model:'qwen/qwen3.6-27b',messages:[{role:'system',content:'Return JSON only.'},{role:'user',content:prompt}],temperature:0.1}),signal:AbortSignal.timeout(8000)});const d:any=await r.json();text=String(d?.choices?.[0]?.message?.content||'')}}catch{} }
      let decision: any = severityFor(riskScore, defaultAntiCheatConfig); let confidence=0; let reason='قرار محلي متعدد الإشارات'
      try { const j=JSON.parse(text.replace(/```json|```/g,'').trim()); if (['NORMAL','WARNING','SUSPICIOUS','CHEATING_DETECTED'].includes(j.decision)) decision=j.decision; confidence=Math.max(0,Math.min(1,Number(j.confidence)||0)); reason=String(j.reason||reason).slice(0,240) } catch {}
      return NextResponse.json({ ok:true, decision, confidence, reason, aiUsed:Boolean(text) })
    }
